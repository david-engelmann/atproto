...
module Moderation = struct
  ...
  let create_report (s : Session.session) (reason_type : string) (reason : string) (subject : string) : string =
    ...
    let data = Printf.sprintf "{\"reasonType\": \"%s\", \"reason\": \"%s\", \"subject\": \"%s\"}" reason_type reason subject in
