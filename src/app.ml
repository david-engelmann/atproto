...
module App = struct
  ...
  let get_profile (s : session) (actor : string) =
    ...
    let sesh = Session.create_session username password in
    let bearer_token = Session.bearer_token_from_session sesh in
    let application_json = Cohttp_client.application_json_setting_tuple in
    let headers = Cohttp_client.create_headers_from_pairs [application_json; bearer_token] in
    let get_profile_url = sesh.atp_host ^ "/" ^ base_endpoint ^ "app.bsky.actor.getProfile" in
