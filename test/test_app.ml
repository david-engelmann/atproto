...
let test_create_base_url _ =
  ...
  OUnit2.assert_equal
    ~printer:(fun x -> x)
    (Auth.origin_of_host test_session.atp_host ^ "/xrpc/")
    test_base_url
...
let test_create_endpoint_url _ =
  ...
  OUnit2.assert_equal
    ~printer:(fun x -> x)
    (Auth.origin_of_host test_session.atp_host ^ "/xrpc/test")
    test_endpoint_url

let expected_public_base host = Auth.origin_of_host host ^ "/xrpc/"

let test_create_public_base_url _ =
  let test_base_url = App.create_public_base_url ~host:"bsky.social" () in
  OUnit2.assert_equal
    ~printer:(fun x -> x)
    (expected_public_base "bsky.social")
    test_base_url
...
let test_create_public_endpoint_url _ =
  ...
  OUnit2.assert_equal
    ~printer:(fun x -> x)
    (expected_public_base "bsky.social" ^ "test")
    test_endpoint_url
