...
let test_sample_session_atp_host _ =
  match sample_session with
  | { atp_host; _ } -> OUnit2.assert_equal "https://bsky.social" atp_host

let test_sample_session_auth_token _ =
  match sample_session with
  | { auth = { token; _ }; _ } -> OUnit2.assert_equal "JWToken" token
...
let test_sample_session_auth_refresh_token _ =
  ...
  | { auth = { refresh_token; _ }; _ } ->
      OUnit2.assert_equal
        "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzY29wZSI6ImNvbS5hdHByb3RvLnJlZnJlc2giLCJzdWIiOiJkaWQ6cGxjOnhvdjN1dnhmZDR0bzZldjNhazVnNXV4ayIsImp0aSI6InM0Z2JDcWRXRlVhQ1lJQk4xdk93V2xBS01LR3ZkSnlla1V3TjJKL1paUDQiLCJpYXQiOjE2ODcyODgzMjIsImV4cCI6MTY5NTA2NDMyMn0.2wdx89mPzrwVyFHhVOpHw6iIooFCE3k6a4qvvBNwcCE"
        (Option.get refresh_token)

let test_sample_session_auth_did _ =
  match sample_session with
  | { auth = { did; _ }; _ } -> OUnit2.assert_equal "did_public_key" did
...
let test_create_session _ =
  ...
  OUnit2.assert_equal ~printer:string_of_bool true
    (String.length test_session.username > 0);
  OUnit2.assert_equal ~printer:string_of_bool true
    (String.length test_session.password > 0);
  OUnit2.assert_equal ~printer:string_of_bool true
    (String.length test_session.atp_host > 0);
  OUnit2.assert_equal ~printer:string_of_bool true
    (String.length test_session.auth.token > 0);
  OUnit2.assert_equal ~printer:string_of_bool true
    (String.length test_session.auth.did > 0)
...
let suite =
  ...
  >::: [
         ...
         "test_sample_session_auth_refresh_token"
         >:: test_sample_session_auth_refresh_token;
