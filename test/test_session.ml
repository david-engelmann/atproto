...
let test_sample_session_auth_token _ =
  match sample_session.auth with
  | Some { token; _ }
    -> OUnit2.assert_equal "JWToken" token
  | None -> OUnit2.assert_failure "Auth token is None"
...
let test_sample_session_auth_did _ =
  match sample_session.auth with
  | Some { did; _ } -> OUnit2.assert_equal "did_public_key" did
  | None -> OUnit2.assert_failure "Auth token is None"

let test_create_session _ =
  let test_session = create_test_session () |> Session.refresh_session_auth in
  let test_auth = Option.get test_session.auth in
