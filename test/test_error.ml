...
let test_of_body_error _ =
  match Error.of_body {|{"error":"HandleNotFound","message":"Unable to resolve handle"}|}
  with
  | None -> OUnit2.assert_failure "expected error object"
  | Some e ->
      OUnit2.assert_equal ~printer:(fun x -> x) "HandleNotFound" e.error

let test_to_string _ =
  OUnit2.assert_equal ~printer:(fun x -> x) "Nope: bad"
