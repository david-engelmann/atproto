...
| Notifications | `Notification` | All `listNotifications` known reasons; `getUnreadCount` via `Client.get_json`; `updateSeen` Yojson `update_seen_body` via `Client.post_json` (empty output stays `""`); prefs / prefs-v2 / activity subscriptions / register+unregister push |
