...
**What 0.1.0 covers.** The packaged client through [#151](https://github.com/david-engelmann/atproto/pull/151): XRPC, official lexicon pin `5c154f9c` ([#132](https://github.com/david-engelmann/atproto/pull/132)) with a coverage gate, repo sync, identity, AppView, Ozone, chat client, live local OAuth/DPoP, Jetstream v2 (`xrpc.v1.json` + dict-zstd), remaining function-level odoc through [#151](https://github.com/david-engelmann/atproto/pull/151), live leftover TestNetwork `getListBlocks` / `getStarterPack` / `getFeedGenerator` / `putActivitySubscription` / PLC request-sign-submit / ozone queue assign / report get-close-stats / setting upsert-remove ([#123](https://github.com/david-engelmann/atproto/pull/123)), remaining served leftover NSIDs (`refreshIdentity` / `checkHandleAvailability` / `dereferenceScope` / `resolveLexicon` / throwaway `requestAccountDelete` / `deleteAccount` / `revokeAppPassword` when create is not the known 500, unspecced skeletons / `getSuggested*` / `getTrendsSkeleton`, `app.bsky.ageassurance.*`, ozone team / signature / hosting / queue unassign-route / report assign-activities-reassign-refresh / `set.deleteValues`; [#129](https://github.com/david-engelmann/atproto/pull/129)), live `tools.ozone.moderation.getAccountPreferences` ([#137](https://github.com/david-engelmann/atproto/pull/137)), live leftover TestNetwork hops for remaining `com.atproto.admin` NSIDs ([#150](https://github.com/david-engelmann/atproto/pull/150)), and odoc HTML on GitHub Pages (https://david-engelmann.github.io/atproto/, enabled after [#112](https://github.com/david-engelmann/atproto/pull/112)). Pin this GitHub repo; it is **not** on opam-repository. See [CHANGELOG.md](CHANGELOG.md).
