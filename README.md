...
Version notes for **0.1.0** (the packaged surface through [#177](https://github.com/david-engelmann/atproto/pull/177): notes hygiene [#175](https://github.com/david-engelmann/atproto/pull/175), Error on the odoc landing map plus libzstd install notes [#176](https://github.com/david-engelmann/atproto/pull/176), and Client HTTP/2 / Feed filter odoc) are in [CHANGELOG.md](CHANGELOG.md). Module HTML from `dune build @doc` is uploaded as the `odoc-html` TestSuite artifact on pull requests. On push to `main`, the same HTML is deployed with GitHub Actions Pages (`actions/upload-pages-artifact` + `actions/deploy-pages`) to https://david-engelmann.github.io/atproto/. GitHub Pages is enabled (Settings → Pages → Source: GitHub Actions). `dune-project` `documentation` points at that live URL.
