...
Version notes for **0.1.0** (the packaged surface through [#185](https://github.com/david-engelmann/atproto/pull/185): typed Actor `putPreferences` encoding on top of [#184](https://github.com/david-engelmann/atproto/pull/184): notes hygiene [#180](https://github.com/david-engelmann/atproto/pull/180) / [#183](https://github.com/david-engelmann/atproto/pull/183), lexicon pin `f0d4877a` for actor interests `updatedAt` [#181](https://github.com/david-engelmann/atproto/pull/181), merge-when-green ignoring lexicon-pin drift [#182](https://github.com/david-engelmann/atproto/pull/182), leftover `interestsPref.updatedAt` live hops [#184](https://github.com/david-engelmann/atproto/pull/184), and `preference_kind_to_json` / `put_preferences_typed` so clients can round-trip `interests_pref.updated_at` without hand-building Yojson; green non-draft same-repo/Dependabot PRs to `main` may auto-squash-merge, see [CONTRIBUTING](.github/CONTRIBUTING.md)) are in [CHANGELOG.md](CHANGELOG.md). Module HTML from `dune build @doc` is uploaded as the `odoc-html` TestSuite artifact on pull requests. On push to `main`, the same HTML is deployed with GitHub Actions Pages (`actions/upload-pages-artifact` + `actions/deploy-pages`) to https://david-engelmann.github.io/atproto/. GitHub Pages is enabled (Settings → Pages → Source: GitHub Actions). `dune-project` `documentation` points at that live URL.
