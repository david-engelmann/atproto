...
- Hosting a **public HTTPS client-metadata document** and completing a **production browser login** against a remote PDS. Local TestNetwork now serves a loopback metadata document and runs discovery + PAR + DPoP in CI (`test/test_local_oauth.ml`). The local AS may still require a browser consent HTML page for the authorization code; token is exercised only when the oauth-provider sign-in/consent APIs complete with `alice.test` / `hunter2`.
