...
**What 0.1.0 covers.** The packaged client through [#160](https://github.com/david-engelmann/atproto/pull/160): XRPC, official lexicon pin `5c154f9c` ([#132](https://github.com/david-engelmann/atproto/pull/132)) with a coverage gate, repo sync, identity, AppView, Ozone, chat client, live local OAuth/DPoP, Jetstream v2 (`xrpc.v1.json` + dict-zstd), remaining function-level odoc through [#160](https://github.com/david-engelmann/atproto/pull/160), live leftover TestNetwork `getListBlocks` / `getStarterPack` / `getFeedGenerator` / `putActivitySubscription` / PLC request-sign-submit / ozone queue assign / report get-close-stats / setting upsert-remove ([#123](https://github.com/david-engelmann/atproto/pull/123)), remaining served leftover NSIDs (`refreshIdentity` / `checkHandleAvailability` / `dereferenceScope` / `resolveLexicon` / throwaway `requestAccountDelete` / `deleteAccount` / `revokeAppPassword` when create is not the known 500, unspecced skeletons / `getSuggested*` / `getTrendsSkeleton`, `app.bsky.ageassurance.*`, ozone team / signature / hosting / queue unassign-route / report assign-activities-reassign-refresh / `set.deleteValues`; [#129](https://github.com/david-engelmann/atproto/pull/129)), live `tools.ozone.moderation.getAccountPreferences` ([#137](https://github.com/david-engelmann/atproto/pull/137)), live leftover TestNetwork hops for remaining `com.atproto.admin` NSIDs ([#150](https://github.com/david-engelmann/atproto/pull/150)), leftover `com.atproto.server` (`createInviteCode(s)` / email confirm / password reset; [#152](https://github.com/david-engelmann/atproto/pull/152)), AppView `sendInteractions` / `describeFeedGenerator` / notification `putPreferences` v1 ([#153](https://github.com/david-engelmann/atproto/pull/153)), and ozone `tools.ozone.report.getAssignments` / `unassignModerator` ([#154](https://github.com/david-engelmann/atproto/pull/154)) hops, and odoc HTML on GitHub Pages (https://david-engelmann.github.io/atproto/, enabled after [#112](https://github.com/david-engelmann/atproto/pull/112)). Hosted-only `chat.bsky.*` / video transcoder / Tap / phone / contacts / push / unhosted feed generator stay listed not faked. Pin this GitHub repo; it is **not** on opam-repository. See [CHANGELOG.md](CHANGELOG.md).
