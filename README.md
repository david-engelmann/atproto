...
**0.1.0** is the first public opam release candidate. It is maintained for third-party use. See [CHANGELOG.md](CHANGELOG.md) for the packaged surface (through [#235](https://github.com/david-engelmann/atproto/pull/235)) and [Remaining gaps](#remaining-gaps) for hosted-only products this client does not fake.
...
Version notes for **0.1.0** are in [CHANGELOG.md](CHANGELOG.md) (`## [0.1.0] - 2026-09-09`). The packaged surface is the protocol client through [#235](https://github.com/david-engelmann/atproto/pull/235); hosted-only products stay listed under [Remaining gaps](#remaining-gaps). No lexicon pin bump (official pin stays `f0d4877a`).
...
**What 0.1.0 covers.** See [CHANGELOG.md](CHANGELOG.md) for the packaged client through [#235](https://github.com/david-engelmann/atproto/pull/235) (OAuth, hosted chat/video clients, TAP-like `Repo_sync`, Jetstream archive token env, phone/contacts/push clients, Client XRPC migration, Ozone helpers, OCaml 5 packaging). Hosted-only SMS / APNs-FCM / unhosted feed generator stay listed not faked (`requestPhoneVerification` is not faked). Install with `opam install atproto` once the opam-repository PR merges; pin this GitHub repo for development.
