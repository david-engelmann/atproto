...
`test/test_local_pds.ml` hits PDS `com.atproto` identity / session / repo / blob / sync. `test/test_local_appview.ml` hits AppView `app.bsky.actor` / `feed` / `graph` (public reads on `:2584`). Authenticated product APIs (`getTimeline`, `getMutes`, `listNotifications`) go through the PDS, which proxies to this AppView — local AppView rejects PDS `at+jwt` access tokens (`InvalidToken: Malformed token`), which is how official clients work too. `test/test_local_ozone.ml` hits `tools.ozone.moderation.emitEvent` / `queryEvents` / `queryStatuses`, `tools.ozone.server.getConfig`, and `com.atproto.label.queryLabels` via the PDS + `atproto-proxy` (direct Ozone also rejects `at+jwt`). If the local network is up, a failed protocol call **fails the test**. The suite skips only when it is not aimed at a local host (typical laptop `dune runtest` without Docker/Node). In CI, `ATP_REQUIRE_LOCAL_PDS=1` is set and the stack is required.
